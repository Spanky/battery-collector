...
void ABatteryCollectorCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Collect", IE_Pressed, this, &ABatteryCollectorCharacter::CollectionNearbyPickups);
...
void ABatteryCollectorCharacter::CollectionNearbyPickups()
{
	TArray<AActor*> overlappingActors;
	CollectionSphere->GetOverlappingActors(overlappingActors);

	float collectedPower = 0.0f;

	for(AActor* overlappedActor : overlappingActors)
	{
		APickup* pickup = Cast<APickup>(overlappedActor);
...
{
	CurrentPower = FMath::Max(CurrentPower + PowerChange, 0.0f);
